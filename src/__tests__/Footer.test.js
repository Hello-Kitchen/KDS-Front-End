...
describe('Footer Component', () => {
    test('renders buttons based on buttons prop', () => {
        ...
        const { rerender } = render(
            <MemoryRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}><Footer buttons={buttons} setConfig={mockSetConfig} activeTab="" updateActiveTab={updateActiveTabMock} /></MemoryRouter>
        );
    ...
    test('renders unknown button', () => {
        ...
        render(<MemoryRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}><Footer buttons={buttons} setConfig={mockSetConfig} activeTab="" updateActiveTab={updateActiveTabMock} /></MemoryRouter>);

        // Ensure the unknown button does not render
    ...
    test('renders connection status as connected', async () => {
        ...
        render(
            <MemoryRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}><Footer buttons={['servie', 'precedent', 'suivant']} setConfig={mockSetConfig} activeTab="" updateActiveTab={updateActiveTabMock} /></MemoryRouter>
        );
